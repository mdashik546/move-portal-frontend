...
import { Play, Star, Award, Zap, Plus } from "lucide-react";
import { UserRole } from "@/lib/authUtlis";
import { useRouter } from "next/navigation";
interface UserInfoProps {
  userInfo?: { role?: UserRole };
}
function HeroSection({ userInfo }: UserInfoProps) {
        ...
        <div className="w-full lg:w-[60%] space-y-10">
          ...
          <div className="flex flex-wrap items-center gap-x-6 gap-y-3">
            ...
            {metadata.map((item, i) => (
              <div
                key={i}
                className="flex items-center gap-2 text-xs font-medium text-zinc-500 uppercase tracking-[0.2em]"
              >
          ...
          <div className="relative space-y-1">
            <span className="absolute -left-12 top-0 text-[10px] font-black uppercase tracking-[0.5em] [writing-mode:vertical-lr] text-primary animate-pulse hidden lg:block">
              Now Streaming
            </span>
            <p className="text-sm font-bold uppercase tracking-[0.3em] text-zinc-500">
              Genesis Originals Presents
            </p>
          ...
          <div className="flex flex-wrap items-center gap-6 pt-6">
            ...
            {userInfo?.role === UserRole.USER ? (
              <button
                ...
                className="flex items-center justify-center gap-4 sm:rounded-xl rounded-lg border border-white/5 bg-zinc-950/50 sm:px-10 px-5 sm:py-5 py-3 text-sm font-black uppercase tracking-widest text-white backdrop-blur-lg transition-all hover:bg-white/5 active:scale-95 hover:border-white/10"
              >
